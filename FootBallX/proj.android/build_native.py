...
def do_build(footballx_root, cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode):
    ndk_path = os.path.join(ndk_root, "ndk-build")

    # windows should use ";" to seperate module paths
    platform = sys.platform
    if platform == 'win32':
        ndk_module_path = 'NDK_MODULE_PATH=%s;%s/external;%s/cocos;%s' % (cocos_root, cocos_root, cocos_root, footballx_root)
    else:
        ndk_module_path = 'NDK_MODULE_PATH=%s:%s/external:%s/cocos:%s' % (cocos_root, cocos_root, cocos_root, footballx_root)
...
def build(ndk_build_param,android_platform,build_mode):
    ...
    footballx_root = os.path.join(current_dir, "../..")
    ...
    do_build(footballx_root, cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode)
